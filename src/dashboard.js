...
// dashboard runs in legacy unauthenticated mode (warning logged at boot).
const THALAMUS_API_KEY = (process.env.THALAMUS_API_KEY || "").trim();
const AUTH_REQUIRED = THALAMUS_API_KEY.length > 0;
if (!AUTH_REQUIRED) {
  console.warn("[thalamus-dashboard] THALAMUS_API_KEY not set — running in LEGACY UNAUTH mode");
} else {
  console.log("[thalamus-dashboard] auth: bearer/key required, key_sha8=" +
    crypto.createHash("sha256").update(THALAMUS_API_KEY).digest("hex").slice(0, 8));
}
...
function checkAuth(req, url) {
  ...
  // Skill probe free pass: /api/resolve?packet_id=__skillprobe__ — returns ok:false fast,
  // no info leak (packet_store regex rejects non-matching id).
  if (url.pathname === "/api/resolve" && url.searchParams.get("packet_id") === "__skillprobe__") return true;
  const auth = String(req.headers["authorization"] || "");
  if (auth.startsWith("Bearer ")) {
    if (timingSafeEqualStr(auth.slice(7).trim(), THALAMUS_API_KEY)) return true;
  }
  const qkey = url.searchParams.get("key");
  if (qkey && timingSafeEqualStr(qkey, THALAMUS_API_KEY)) return true;
  return false;
}
...
async function handle(req, res) {
  try {
    ...
    // PRD-F: gate /api/* routes behind bearer/key auth when THALAMUS_API_KEY is set.
    if (url.pathname.startsWith("/api/") && !checkAuth(req, url)) {
      res.writeHead(401, { "content-type": "application/json; charset=utf-8" });
      res.end(JSON.stringify({ ok: false, error: "unauthorized: provide Authorization: Bearer <key> or ?key=<key>" }));
      return;
    }
